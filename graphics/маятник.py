...
coords = gr.Point(center_point.x + length * m.sin(angle), center_point.y + length * m.cos(angle))  # Начальные координаты шара
# Фон
...
ball.draw(window)
# Рисуем веревку
T=gr.Point(coords.x, coords.y)
line = gr.Line(gr.Point(SIZE_X / 2, SIZE_Y / 2),T)
line.setFill('black')
line.draw(window)
